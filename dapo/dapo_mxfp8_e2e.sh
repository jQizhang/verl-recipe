...
# End-to-end MXFP8 variant of dapo_mxfp8.sh:
#   - Training side (actor / Megatron + TE): MXFP8 enabled
#   - Rollout / inference side (SGLang):     MXFP8 enabled
# The only difference from dapo_mxfp8.sh is that the actor-side MXFP8
# param-gather / grad-reduce overlap switches default to True here so the
# full training-side MXFP8 path (matching run_dapo_qwen_moe_30b_megatron_mxfp8e2e.sh
# with ACTOR_FP8=1) is on out of the box. All values remain env-overridable.

ID=${1:-"dapo-qwen3-8b-megatron-mxfp8-e2e"}
HOME_DIR=/apps

project_name=${PROJECT_NAME:-DAPO-mxfp8-e2e-rebase-4n}
exp_name=$ID
...
# ---- Training side (actor / Megatron + TE) MXFP8 ----
# Defaults flipped to True vs dapo_mxfp8.sh to enable the full e2e training path.
actor_fp8_recipe=${ACTOR_FP8_RECIPE:-mxfp8}
actor_fp8_param=${ACTOR_FP8_PARAM:-False}
actor_first_last_layers_bf16=${ACTOR_FIRST_LAST_LAYERS_BF16:-True}
actor_num_layers_at_start_in_bf16=${ACTOR_NUM_LAYERS_AT_START_IN_BF16:-1}
actor_num_layers_at_end_in_bf16=${ACTOR_NUM_LAYERS_AT_END_IN_BF16:-1}
actor_fp8_param_gather=${ACTOR_FP8_PARAM_GATHER:-True}
actor_reuse_grad_buf_for_mxfp8_param_ag=${ACTOR_REUSE_GRAD_BUF_FOR_MXFP8_PARAM_AG:-True}
actor_overlap_grad_reduce=${ACTOR_OVERLAP_GRAD_REDUCE:-True}
actor_overlap_param_gather=${ACTOR_OVERLAP_PARAM_GATHER:-True}
...
MXFP8_TRAINING=(
    ...
    +actor_rollout_ref.actor.megatron.override_ddp_config.overlap_grad_reduce=${actor_overlap_grad_reduce}
    +actor_rollout_ref.actor.megatron.override_ddp_config.overlap_param_gather=${actor_overlap_param_gather}
    ...
    +actor_rollout_ref.actor.optim.override_optimizer_config.overlap_param_gather=${actor_overlap_param_gather}
